...
export default function DomeGallery({
  ...
  useEffect(() => {
    ...
    const rotateStep = (time: number) => {
      ...
      if (!draggingRef.current && !openingRef.current && rootRef.current?.getAttribute("data-enlarging") !== "true") {
        rotationRef.current.y += (autoRotateSpeed * dt) / 16.66;
  ...
  useEffect(() => {
    ...
    const close = () => {
      ...
      if (originalImg) {
        ...
        img.style.cssText = "width: 100%; height: 100%; object-fit: contain; padding: 1rem;";
  ...
  const openItemFromElement = (el: HTMLElement) => {
    ...
    overlay.style.cssText = `position:absolute; left:${
      ...
      frameR.height
    }px; opacity:0; z-index:30; will-change:transform,opacity; transform-origin:top left; transition:transform ${enlargeTransitionMs}ms ease, opacity ${enlargeTransitionMs}ms ease; border-radius:${openedImageBorderRadius}; overflow:hidden; box-shadow:0 10px 30px rgba(0,0,0,.35); background-color: #111; display:flex; justify-content:center; align-items:center;`;
    const rawSrc =
    ...
    img.src = rawSrc;
    img.alt = rawAlt;
    // Tweaked for logos/skills
    img.style.cssText = `width:100%; height:100%; object-fit:contain; padding:2rem; filter:${
      grayscale ? "grayscale(1)" : "none"
    };`;
  ...
  const cssStyles = `
    ...
    /* Removed body.dg-scroll-lock to prevent breaking main page scroll incorrectly since DomeGallery will be used in a section */
    .item__image {
      position: absolute;
      inset: 10px;
    ...
    <>
      ...
      >
        ...
        >
          ...
          <div
            ref={viewerRef}
            className="absolute inset-0 z-20 pointer-events-none flex items-center justify-center p-8 md:p-16"
